                ...
                <>
                  <div className={styles.platesHeader}>
                    <div>
                      ...
                      {selectedList.description && <p className={styles.platesSubtitle}>{selectedList.description}</p>}
                    </div>
                    
                    <div className={styles.platesControls}>
                      <div className={styles.searchBox}>
                        <i className={`ri-search-line ${styles.searchIcon}`}></i>
                        <input type="text" placeholder="Поиск по номеру или марке..." value={searchTerm} onChange={handleSearch} className={styles.searchInput} />
                        {searchTerm && <button onClick={() => setSearchTerm('')} className={styles.clearSearch}><i className="ri-close-line"></i></button>}
                      </div>
                      <button onClick={handleRefresh} className={styles.refreshButton} disabled={platesLoading}>
                  ...
                  {platesLoading ? (
                    <div className={styles.platesLoading}><div className={styles.spinner}></div><p>Загрузка номеров...</p></div>
                  ) : filteredPlates?.length > 0 ? (
                    <div className={styles.platesGrid}>
                      {filteredPlates.map((plate) => {
                        const active = isPlateActive(plate);
                        return (
                          <div key={plate.id} className={`${styles.plateCard} ${!active ? styles.plateCardInactive : ''}`}>
                            ...
                            <div className={styles.plateDetails}>
                              {(plate.vehicleBrand || plate.vehicleModel) && (
                                <div className={styles.detailRow}>
                                  ...
                                  <span>{plate.vehicleBrand} {plate.vehicleModel}{plate.vehicleColor && ` (${plate.vehicleColor})`}</span>
                                </div>
                              )}
                              {plate.validFrom && (
                                <div className={styles.detailRow}><i className="ri-calendar-check-line"></i><span>с {formatDate(plate.validFrom)}</span></div>
                              )}
                              {plate.validUntil ? (
                                <div className={styles.detailRow}>
                                  <i className="ri-calendar-close-line"></i>
                                  <span className={new Date(plate.validUntil) < new Date() ? styles.expiredDate : ''}>до {formatDate(plate.validUntil)}</span>
                                </div>
                              ) : (
                                <div className={styles.detailRow}><i className="ri-calendar-line"></i><span>бессрочно</span></div>
                              )}
                              {plate.notes && (
                                <div className={styles.detailRow}><i className="ri-file-text-line"></i><span className={styles.notes}>{plate.notes}</span></div>
                              )}
                            </div>
                            
                            <div className={styles.plateFooter}>
                              <span className={styles.createdAt}>Добавлен: {formatDate(plate.createdAt)}</span>
                              <button type="button" onClick={() => openDeleteModal(plate.id, plate.plateNumber)} className={styles.deleteButton} title="Удалить номер из списка"
                                style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', backgroundColor: '#fef2f2', color: '#dc2626', border: '1px solid #fee2e2', borderRadius: '6px', fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}>
                    ...
                    <div className={styles.emptyPlates}>
                      ...
                      <h3>Нет номеров</h3>
                      <p>{searchTerm ? 'По вашему запросу ничего не найдено' : 'В этом списке пока нет номеров вашей организации'}</p>
                      {searchTerm && <button onClick={() => setSearchTerm('')} className={styles.clearSearchButton}><i className="ri-close-line"></i>Сбросить поиск</button>}
                    </div>
                  )}
                </>
              )}
        ...
        <div className={styles.modalOverlay} onClick={closeDeleteModal}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}><i className="ri-delete-bin-line"></i>Удаление номера из списка</h3>
              <button onClick={closeDeleteModal} className={styles.modalClose}><i className="ri-close-line"></i></button>
            </div>
            <div className={styles.modalBody}>
              <div className={styles.warningBox}>
                <i className="ri-alert-line"></i>
                <p>Вы собираетесь удалить номер <strong>{deleteModal.plateNumber}</strong> из списка пропусков. После удаления машина не сможет проехать через КПП.</p>
              </div>
              <div className={styles.formGroup}>
                <label htmlFor="deleteReason" className={styles.label}>Причина удаления <span className={styles.required}>*</span></label>
                <textarea id="deleteReason" value={deleteModal.reason} onChange={(e) => setDeleteModal(prev => ({ ...prev, reason: e.target.value }))} className={styles.textarea} placeholder="Например: сотрудник уволился, продал машину и т.д." rows={4} autoFocus />
              </div>
            </div>
            <div className={styles.modalFooter}>
              <button onClick={closeDeleteModal} className={styles.cancelButton} disabled={deleteModal.loading}>Отмена</button>
              <button onClick={handleDeletePlate} disabled={deleteModal.loading || !deleteModal.reason.trim()} className={styles.submitButton} style={{ backgroundColor: '#dc2626' }}>
                {deleteModal.loading ? <><i className="ri-loader-4-line ri-spin"></i><span>Удаление...</span></> : <><i className="ri-delete-bin-line"></i><span>Удалить номер</span></>}
              </button>
            </div>
          </div>
        </div>
